...
//  History:
//  2021-10-18  1.0  new
//  2024-06-13  Modified for NT1066 Evaluation Kit
//
#pragma NOIV
...
#define VER_FW       0xA0       // Firmware version for NT1066
#ifndef F_TCXO
...
#define MOSI 0 // PD0 -> NT1066 MOSI
#define MISO 1 // PD1 <- NT1066 MISO
#define CSN  3 // PD3 -> NT1066 CSN
...
static uint8_t read_sdata(void) {
  ...
  uint8_t dat = digitalRead(MISO);
  write_sclk();
  return dat;
}

// write NT1066 SPI frame header -----------------------------------------------
static void write_head(uint16_t addr, uint8_t mode) {
  int8_t i;

  write_sdata(mode); // 0:write,1:read
  write_sdata(0); // ainc=0:single
